...
import { Governor, Proposal, ProposalCall, ProposalSupport, VoteReceipt } from "../generated/schema"
import { RethinkFundGovernor } from "../generated/templates/RethinkFundGovernor/RethinkFundGovernor"
import { fetchAccount } from "./Account.util"

export function fetchGovernor(address: Address): Governor {
	let contract = Governor.load(address)

	if (contract == null) {
		const COUNTING_MODE = RethinkFundGovernor.bind(address).try_COUNTING_MODE()
